...
async def _run_sandboxed(
    ...
    """Route a bash/python invocation through the configured sandbox, in the
    per-session workspace. FAILS CLOSED: if the requested backend is unavailable
    on this host it returns an error, never degrading to unsandboxed execution.
    Maps the SandboxResult onto the tool's {output|error, exit_code} shape."""
    ...
    try:
        sandbox = get_sandbox(backend)
    except SandboxUnavailable as exc:
        return {"error": f"sandbox backend '{backend}' unavailable: {exc}", "exit_code": 1}
    ...
    if res.timed_out:
        return {
            ...
            "exit_code": 124,
            "stdout": _truncate(res.stdout, MAX_OUTPUT_CHARS),
            "stderr": _truncate(res.stderr, MAX_OUTPUT_CHARS),
        }
...
async def _direct_fallback(
    ...
    try:
        if tool == "bash":
            _sbx = _sandbox_backend()
            if _sbx:
                return await _run_sandboxed(_sbx, content, DEFAULT_BASH_TIMEOUT, session_id)
