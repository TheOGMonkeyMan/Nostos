"""Living-document tool handlers + the active-document state.

Extracted verbatim from src/tool_implementations.py (Phase 2.2 / ADR-035).
Behavior is unchanged. This module OWNS the active-document state
(_active_document_id / _active_model module globals); the set/get accessors
and the do_*_document handlers live here, and are re-exported from
src.tool_implementations so callers (agent_loop, agent_tools, chat_routes,
document_routes, pdf_form_doc, tool_execution, management.py) keep working.
This module owns its own logger (name src.tools.documents).
"""
