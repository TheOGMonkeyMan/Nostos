"""Deep-research tool handlers.

Extracted verbatim from src/tool_implementations.py (Phase 2.2 / ADR-031).
Behavior is unchanged; these handlers are re-exported from
src.tool_implementations so existing import paths keep working. _parse_tool_args
(and, for do_trigger_research, the shared cookbook constants _COOKBOOK_BASE and
_internal_headers) are imported lazily to avoid a circular import with the
tool_implementations module.
"""
...
async def do_trigger_research(content: str, owner: Optional[str] = None) -> Dict:
    ...
    import httpx
    from src.tool_implementations import _parse_tool_args, _COOKBOOK_BASE, _internal_headers
    try:
