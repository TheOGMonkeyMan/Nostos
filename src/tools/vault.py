"""Bitwarden / Vaultwarden CLI tool handlers.

Extracted verbatim from src/tool_implementations.py as the first Phase 2.2
decomposition slice (ADR-029). Behavior is unchanged; these handlers are
re-exported from src.tool_implementations so existing import paths keep
working. The shared _parse_tool_args helper is imported lazily inside each
handler to avoid a circular import with the tool_implementations module.
"""
